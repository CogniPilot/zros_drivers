...
LOG_MODULE_REGISTER(sense_baro, CONFIG_ZROS_SENSE_BARO_LOG_LEVEL);
...
typedef struct context_t {
	...
	struct zros_pub pub;
	// devices
	const struct device *baro_dev[CONFIG_ZROS_SENSE_BARO_COUNT];
	// raw readings
	double baro_raw[CONFIG_ZROS_SENSE_BARO_COUNT];
} context_t;
...
static context_t g_ctx = {
	...
	.altimeter =
		{
			.has_header = true,
			.header = {.frame_id = "wgs84", .has_stamp = true, .seq = 0},
			.vertical_position = 0,
...
void baro_work_handler(struct k_work *work_item)
{
	...
	double baro_data_array[CONFIG_ZROS_SENSE_BARO_COUNT][2] = {};
	for (int i = 0; i < CONFIG_ZROS_SENSE_BARO_COUNT; i++) {
		...
		if (ctx->baro_dev[i] != NULL) {
			...
			LOG_DBG("baro %d: %d.%06d %d.%06d", i, baro_press.val1, baro_press.val2,
...
K_THREAD_DEFINE(sense_baro, MY_STACK_SIZE, sense_baro_entry_point, &g_ctx, NULL, NULL, MY_PRIORITY,
		0, 100);
