...
struct context {
	...
	bool running;
	synapse_pb_Vector3 data;
	struct {
		double optical_flow[3];
	} raw;
	struct {
		struct rtio *ctx;
		       ...
		       struct sensor_three_axis_data *out)
{
	...
	ret = decoder->decode(buf, (struct sensor_chan_spec) {ch, 0}, &fit, 1, out);
	if (ret < 0) {
		return ret;
	}
...
static void optical_flow_publish(struct context *ctx)
{
	stamp_msg(&ctx->data.stamp, k_uptime_ticks());
	ctx->data.x = ctx->raw.optical_flow[0];
	ctx->data.y = ctx->raw.optical_flow[1];
	ctx->data.z = ctx->raw.optical_flow[2];
...
static void process_events(int result, uint8_t *buf, uint32_t len, void *userdata)
{
	...
	struct sensor_three_axis_data data = {0};
	int ret;
	...
	ret = decode_data(iodev, buf, SENSOR_CHAN_POS_DXYZ, &data);
	...
	for (size_t i = 0 ; i < 3 ; i++) {
		ctx->raw.optical_flow[i] = Z_SHIFT_Q31_TO_F32(data.readings[0].values[i],
							      data.shift);
	}
	optical_flow_publish(ctx);
}
